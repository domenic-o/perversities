...
\subsubsection{Strictly perverse coherent sheaves}

 Let $X$ be a smooth projective variety over $\mathbb{C}$ (this can be greately weakened) and consider $\mathscr{D} = \mathscr{D}^b(\textrm{Coh}(X))$, the derived category of coherent sheaves on $X$, with its canonical heart $\heartsuit = \textrm{Coh}(X)$. Then there is an abelian $\{0, \cdots , n \}$-slicing on $\heartsuit$ giving by defining $\heartsuit_i$, for each $0 \leq i \leq n$, as the subcategory consisting of sheaves with support of pure dimension $i$. The abelian Postnikov towers simply amount to the classical filtrations by codimension of support. When seen trough the poset embedding $\{0, \cdots , n \} \subseteq \Z$, the Grothendieck vanishing theorem implies that this is a 'weak' grading filtration, in the sense that it doesn't satisty the second requirement from \cite{defgrad}. However, (the proof of) \cite{grad} still applies if we start with a  'strict' perversity i.e., one which satisfies strict inequalities from \cite{defper}. The so obtained bounded $t$-structure on $\mathscr{D}$ coincides with the one of perverse coherent sheaves as in \cite{bezr}. Indeed, using Grothendieck-Verdier duality one can show that every perversity gives rise to such a $t$-structures, even though the given slicing is not grading. Applying  \cite{main-thm} finally gives the 'distributive lattice of $t$-structures' from \cite{bondper}. 
